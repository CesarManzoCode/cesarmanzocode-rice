...
`wallpapers/` ships four final monochrome wallpapers, each 1920x1080+,
procedurally generated by `scripts/dev/generate_wallpaper.py` (Python
standard library only — no Pillow, no ImageMagick, no AI image
generation, no network access). The script is dev-time only: it is never
invoked by `install.sh`/`apply.sh`/`uninstall.sh`, so Python is not a
